...
namespace Codefix.Dataverse.Core.Expressions
{
    internal class ValueExpression
    {
        ...
        protected virtual object GetValueOfMemberExpression(MemberExpression expression) => expression.Expression switch
        {
            ...
            MethodCallExpression callExpression => GetValueOfMethodCallExpression(expression, callExpression),
        ...
        protected virtual object GetValueOfMethodCallExpression(MemberExpression expression, MethodCallExpression methodCallExpression)
        {
            switch (methodCallExpression.Method.Name)
            {
                ...
                case "get_Item":
                    var item = (GetValue(methodCallExpression.Object) as System.Collections.IEnumerable).Cast<object>().ElementAtOrDefault(int.Parse(GetValue(methodCallExpression.Arguments[0]).ToString()));

                    var value = item.GetType().GetProperty(expression.Member.Name).GetValue(item);
