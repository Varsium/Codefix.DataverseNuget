        ...
        protected override string VisitMethodCallExpression(LambdaExpression topExpression, MethodCallExpression methodCallExpression)
        {
            ...
            if (typeof(object).IsAssignableFrom(declaringType))
            {
                switch (methodCallExpression.Method.Name)
                {
                    ...
                    case "get_Item":
                        // Extract the argument expressions (collection and indexer)
                        var collectionExpression = methodCallExpression.Object;
                        var indexerExpression = methodCallExpression.Arguments[0];

                        // Visit the argument expressions to transform if needed
                        var visitedCollectionExpression = VisitExpression(topExpression, collectionExpression);
                        var visitedIndexerExpression = VisitExpression(topExpression, indexerExpression);

                        // Create a new expression to access the element using indexer
                        Expression resultExpression;

                        // Check if the collection is an array
                        //if (collectionExpression.Type.IsArray)
                        //{
                        //    // Use Expression.ArrayIndex for arrays
                        //    resultExpression = Expression.ArrayIndex(visitedCollectionExpression, visitedIndexerExpression);
                        //}
                        //else
                        //{
                        //    // Use Expression.Property for dictionaries or other collections with indexers
                        //    resultExpression = Expression.Property(visitedCollectionExpression, "Item", visitedIndexerExpression);
                        //}

                        return visitedCollectionExpression;
                    case "First":
                    case "FirstOrDefault":
                        // Extract the argument expression (the list)
                        var listExpression = methodCallExpression.Object;

                        // Visit the argument expression to transform it if needed
                        var visitedListExpression = VisitExpression(topExpression, listExpression);

                        // Extract the property name (if exists) or just return the first element
                        var propertyName = "";
                        if (methodCallExpression.Arguments.Count == 2)
                        {
                            propertyName = (methodCallExpression.Arguments[1] as MemberExpression)?.Member.Name ?? "";
                        }

                        // Create a new expression to access the property or return the first element
                        return visitedListExpression;
